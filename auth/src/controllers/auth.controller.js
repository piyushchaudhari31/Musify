...
async function googleAuth(req, res) {
  try {
    ...
    if (isAlreadyExist) {
      const token = jwt.sign(
        { id: isAlreadyExist._id, role: isAlreadyExist.role, fullName: isAlreadyExist.fullName },
        process.env.JWT_SECRET
      );
...
async function LoginUser(req, res) {
  try {
    ...
    const token = jwt.sign(
      { id: isEmail._id, role: isEmail.role, fullName: isEmail.fullName },
      process.env.JWT_SECRET
    );
